...
def match(
    ...
    a_est,
    c_est,
    temporal_threshold=0.85,
    spatial_threshold=0.85,
    x=64,
    y=64,
    plot=True,
):
    ...
    match_indices -= (
        1  # All values start out negative and are updated if a suitable match is found
    )
    count = 0
    ...
    for k in range(a_real.shape[1]):
        ...
        for i in range(len(good_list)):
            ...
            if (
                temporal_similarity > best_temp
                and temporal_similarity > temporal_threshold
            ):
...
def match_nearest_neighbors(
    a_est, c_est, temporal_threshold=0.85, spatial_threshold=0.85, x=64, y=64
):
    """
...
def generate_match_figs_neighbors(
    a_est,
    c_est,
    matches,
    dims,
    folder_location,
    order="C",
    spatial_titles_match=["Spatial footprint", "Nearest Neighbor"],
    show=False,
    normalize=True,
    zoom=False,
    figsize=(60, 30),
...
def save_fig_generate_nearest_match_parallel(my_dict):
    ...
    if int(my_dict["match_value"]) < 0:
        ...
        fig, gs = plot_img_double(
            my_dict["match_ai"],
            my_dict["match_ci"],
            my_dict["orig_ai"],
            my_dict["orig_ci"],
            my_dict["dims"],
            my_dict["spatial_titles_match"],
            order=my_dict["order"],
            labels=labels,
            normalize=my_dict["normalize"],
            zoom=my_dict["zoom"],
            figsize=my_dict["figsize"],
        )
    ...
    fig.savefig(
        "{}/Brightness{:04d} Neuron{}".format(
            my_dict["folder_location"], my_dict["max_brightness"], my_dict["p"]
        )
    )


def generate_match_figs(
    a_real,
    c_real,
    a_est,
    c_est,
        ...
        if int(matches[p]) < 0:
            ...
            if best_ind >= 0:
                ...
                fig, gs = plot_img_single(
                    ...
                    dims,
                    zoom=zoom,
                    spatial_title="Ground Truth Spatial",
            ...
            fig, gs = plot_img_double(
                ...
                orig_ci,
                dims,
                spatial_titles_match,
                zoom=zoom,
            )
            print(ca_utils.cosine_similarity(match_ci, orig_ci))

        fig.savefig(
            "{}/Neuron{} Brightness{}".format(folder_location, p, max_brightness)
        )
...
def plot_img_double(
    ...
    second_norm_spatial = (a_second / np.linalg.norm(a_second)).reshape(
        dims, order=order
    )
    ...
    plot_updated_axis_Y(
        fig, axis_spatialsecond, (x_low, x_high), fontsize=tick_val, skip=1
    )
    plot_updated_axis_Y(
        fig, axis_spatialfirst, (x_low, x_high), fontsize=tick_val, skip=1
    )
    plot_updated_axis_X(
        fig, axis_spatialsecond, (y_low, y_high), fontsize=tick_val, skip=2
    )
...
def match_corr_mat(a_ref, c_ref, a_est, c_est, matches, spatial=False):
    ...
    for k in range(match_matrix.shape[0]):
        for j in range(match_matrix.shape[1]):
            if spatial:
                temp_cos = ca_utils.cosine_similarity(
                    a_est[:, est_indices[k]], a_ref[:, j]
                )
            else:
                temp_cos = ca_utils.cosine_similarity(
                    c_est[:, est_indices[k]], c_ref[:, j]
                )
...
def generate_movie(a_use, c_use, dims, random_values, slice_val=2, seed=999, order=0):
    ...
    np.random.seed(
        seed
    )  # seed for random color generation (to keep it consistent if desired)
    final_movie = np.zeros(
        (dims[0], dims[1], 3, len(slice_val))
    )  # z is number of planes, and 3 is because we use RGB data
    if order == 0:
        order_string = "F"
    elif order == 1:
        order_string = "C"
    for k in range(a_use.shape[1]):
        ...
        neuron_movie = neuron_movie.reshape(
            (dims[0], dims[1], -1), order=order_string
        ).squeeze()

        random_color = random_values[
            k, :
        ].squeeze()  # [random.randint(0,255) for i in range(3)]
...
def generate_movie_maxproj(
    a_use, c_use, dims, random_values, slice_val=2, seed=999, order=0
):
    """
...
def write_mpl_no_compress_comparisons(
    mov_list,
    filename,
    img_types,
    movie_scales,
    ...
    if os.path.exists("tmp"):
        raise FileExistsError(
            "Remove the tmp directory before running this visualization code"
        )
    else:
        save_directory = "tmp"
...
def make_frame_par(curr_data):
    ...
    gs = gridspec.GridSpec(
        ...
        ceil(n_mov / 2),
        height_ratios=(1, 1),
        width_ratios=(tuple([1 for i in range(ceil(n_mov / 2))])),
    )
    ...
    for mdx, mov in enumerate(mov_list):
        ...
        if mdx == 0:
            ax.set_title(
                titles[mdx] + " Frame:{}".format(t + offset_frame), fontsize=titlesize
            )
        else:
...
def reshape_movie_list(my_list, shape, dim1_range, dim2_range, order, min_value=0):
    ...
    for k in my_list:
        ...
        current_array = current_array[
            dim1_range[0] : dim1_range[1], dim2_range[0] : dim2_range[1], :
        ]
...
def standard_demix_vid_m(
    fin_rlt, start_frame, end_frame, dim1_range, dim2_range, filename=None
):
    ...
    reshaped_mov_list = reshape_movie_list(
        my_movie_list,
    ...
    fr = 30
    write_mpl_no_compress_comparisons(
        reshaped_mov_list, filename, img_types, movie_scales, titles=titles, fr=fr
    )
